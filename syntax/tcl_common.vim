...
function! tcl_common#pred_w_switches(word, group, keywords)
    ...
    " Check whether this command has any keywords
    if a:keywords != ''
...
function! tcl_common#pred_w_subcmd(word, incl, keywords)
    let l:syn_group = 'tcl' . a:word . 'Options'
    let l:contains = l:syn_group
    if a:incl != ''
...
call tcl_common#pred_w_switches('catch',     '','"-\(\(no\)\?\(break\|continue\|eval\|exit\|signal\)\)\>"')
call tcl_common#pred_w_switches('exec',      '','"-\(keepnewline\|ignorestderr\)\>"')
call tcl_common#pred_w_switches('fconfigure','','"-\(blocking\|buffering\|buffersize\|encoding\|eofchar\|error\|translation\)\>"')
call tcl_common#pred_w_switches('fcopy',     '','"-\(size\|command\)\>"')
call tcl_common#pred_w_switches('glob',      '','"-\(directory\|join\|nocomplain\|path\|tails\|types\)\>"')
call tcl_common#pred_w_switches('lsearch',   '','"-\(exact\|glob\|regexp\|sorted\|all\|inline\|not\|ascii\|dictionary\|integer\|nocase\|real\|decreasing\|increasing\|subindices\|start\|index\)\>"')
call tcl_common#pred_w_switches('lsort',     '','"-\(ascii\|dictionary\|integer\|real\|increasing\|decreasing\|indicies\|nocase\|unique\|command\|index\)\>"')
call tcl_common#pred_w_switches('open',      '','"-\(mode\|handshake\|queue\|timeout\|ttycontrol\|ttystatus\|xchar\|pollinterval\|sysbuffer\|lasterror\)\>"')
call tcl_common#pred_w_switches('puts',      '','"-\(nonewline\)\>"')
call tcl_common#pred_w_switches('read',      '','"-\(nonewline\)\>"')
call tcl_common#pred_w_switches('regexp',    '','"-\(about\|expanded\|indices\|line\|linestop\|lineanchor\|nocase\|all\|inline\|start\)\>"')
call tcl_common#pred_w_switches('regsub',    '','"-\(all\|expanded\|line\|linestop\|nocase\|start\)\>"')
call tcl_common#pred_w_switches('return',    '','"-\(code\|errorcode\|errorinfo\|level\|options\)\>"')
call tcl_common#pred_w_switches('socket',    '','"-\(server\|myaddr\|myport\|async\|myaddr\|error\|sockname\|peername\)\>"')
call tcl_common#pred_w_switches('source',    '','"-\(encoding\)\>"')
call tcl_common#pred_w_switches('subst',     '','"-\(nocommands\|novariables\|nobackslashes\)\>"')
call tcl_common#pred_w_switches('switch',    '','"-\(exact\|glob\|regexp\|nocase\|matchvar\|indexvar\)\>"')
call tcl_common#pred_w_switches('unload',    '','"-\(nocomplain\|keeplibrary\)\>"')
call tcl_common#pred_w_switches('unset',     '','"-\(nocomplain\)\>"')

" Predicates with sub commands.
call tcl_common#pred_w_subcmd('after',       '','cancel idle info')
call tcl_common#pred_w_subcmd('binary',      '','format scan decode encode')
call tcl_common#pred_w_subcmd('encoding',    '','convertfrom convertto names system')
call tcl_common#pred_w_subcmd('fileevent',   '','readable writable')
call tcl_common#pred_w_subcmd('info',        '','args body cmdcount commands complete default exists frame functions globals hostname level library loaded locals nameofexecutable patchlevel procs script sharedlibextension tclversion vars')
call tcl_common#pred_w_subcmd('lrange',      '','start end')
call tcl_common#pred_w_subcmd('memory',      '','active break info init onexit tag trace validate')
call tcl_common#pred_w_subcmd('seek',        '','start current end')
call tcl_common#pred_w_subcmd('update',      '','idletasks')

" Predicates with subcommands with switches.
call tcl_common#pred_w_switches('names',     'tclArrayNames','"-\(exact\|glob\|regexp\)\>"')
call tcl_common#pred_w_subcmd('array',       'tclArrayNames','anymore donesearch exists get nextelement set size startsearch statistics unset')

call tcl_common#pred_w_switches('add',       'tclClockOptions','"-\(base\|format\|gmt\|locale\|timezone\)\>"')
call tcl_common#pred_w_switches('clicks',    'tclClockOptions','"-\(base\|format\|gmt\|locale\|timezone\)\>"')
call tcl_common#pred_w_switches('format',    'tclClockOptions','"-\(base\|format\|gmt\|locale\|timezone\)\>"')
call tcl_common#pred_w_switches('scan',      'tclClockOptions','"-\(base\|format\|gmt\|locale\|timezone\)\>"')
call tcl_common#pred_w_subcmd('clock',       'tclClockOptions','microseconds milliseconds seconds')

call tcl_common#pred_w_switches('filter',    'tclDictFilter','"\<\(key\|script\|value\)\>"')
call tcl_common#pred_w_subcmd('dict',        'tclDictFilter','append create exists for get incr info keys lappend merge remove replace set size unset update values with')
